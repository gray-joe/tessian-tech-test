...
class Conversation(object):
    def __init__(self, conversation_id: Text, existing_conversation_events: List[Dict]) -> None:
        """Creates a conversation.
        Args:
            existing_conversation_events: Events which happened earlier in this conversation.
        """
        self.conversation_id = conversation_id
        self.existing_conversation_events = existing_conversation_events
        self.number_existing_conversation_events = len(existing_conversation_events)

    def addd_user_message(self, message: Text) -> None:
        self.existing_conversation_events.append({"type": "user", "message": message})

    def add_bot_message(self, bot_messages: Text) -> None:
        self.existing_conversation_events.append({"type": "bot", "message": bot_messages})

    def new_events_dict(self) -> List[Dict]:
        return self.existing_conversation_events[self.number_existing_conversation_events:]


@contextmanager
def conversationPersistence(conversation_id: Text,) -> Generator[Conversation, None, None]:
    ...
    Args:
        conversation_id: The ID of the conversation. This is usually the same as the username.
...
class ChuckNorrisBot:
    def handle_message(self, message_text: Text, conversation: Conversation) -> None:
        ...
        if len(conversation.existing_conversation_events) <= 1:
